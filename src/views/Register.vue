  ...
  <div class="main-container flex-box">
    <div class="form-box">
      <el-row style="height: 100%" justify="space-between">
        <el-col :sm="12" :xs="24" class="flex-box" style="padding: 20px">
          ...
          >
            ...
            <el-form-item label="验证码" prop="userVerifyCode" style="justify-content: space-between!important;">
              ...
              &emsp;
              <el-button type="primary" style="width: 30%" :disabled="!canGetVerifyCode" @click="getVerifyCode">{{ verifyText }}</el-button>
            </el-form-item>
            <div>
        ...
        <el-col :sm="12" :xs="0">
          <el-image style="width: 100%;height: 100%;border-radius: 0 5px 5px 0;" src="https://file.peteralbus.com/assets/blog/imgs/photo/Photo_6553676_DJI_76_jpg_4191951_0_20215595138_photo_original.jpg" fit="cover"/>
        </el-col>
      </el-row>
    </div>
  </div>
...
let registerByPhone=ref(true)

let verifyText=ref('获取')
let time=0
...
const getVerifyCode = () => {
  if(registerByPhone.value)
  {
    ...
    axios.get('/user/applyPhoneVerifyCode?account='+registerForm.userPhone)
        .then((res)=>{
  ...
  else
  {
    ...
    {
      axios.get('/user/applyMailVerifyCode?account='+registerForm.userMail)
          .then((res)=>{
            if(res.data=='haveVerifyCode')
            {
              ElMessageBox.alert('请求验证码过于频繁，请检查收件箱','验证码已发送',{confirmButtonText:'确认'})
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      axios.post('/user/register',qs.stringify(regData.value))
          .then(res=>{
